...
class Typewriterr {
    ...
    constructor(container, options) {
        ...
        if (container) {
            if (typeof container === "string") {
                ...
                this.state.container = containerElement;
            } else {
    ...
    run() {
        ...
        let result = this.options.strings[1];
        console.log(result);

        let char_count = 0;

        let anim_id = setInterval(frame, 60); // Repeat animation frames

        // this.state.container.classList.remove("hidden");

        // Where the animation magic happens
        let that = this;
        function frame() {
            if (char_count <= result.length) {
                const out = that.options.prefix + result.slice(0, char_count);

                // * Update HTML content and blink cursor
                that.state.container.innerHTML =
                    out +
                    // Blink the cursor every 2 characters unless it's typing a space
                    // and keep the cursor shown after the string has been fully typed
                    ((result[char_count] != " " && char_count % 2 == 0) ||
        ...
        // typewriter_played.push(this.container);
    }
}
...
The first animation
new Typewriter("#hello_friend", {
    prefix: "< ",
    cursor: "_",
    strings: ["Hello friend", "Ciao amico", "Привіт друже", "Hola amigo"],
    loop: true,
    speed: 60,
});
*/
